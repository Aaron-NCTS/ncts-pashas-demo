...
export function Catalog() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 md:px-8 py-14">
      <SectionHeading eyebrow={t('pages.catalog.eyebrow')} title={t('pages.catalog.title')} description={t('pages.catalog.description')} />
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {CATEGORIES.map((cat) => (
            ...
            >
              {labelFor(PRODUCT_CATEGORY_LABEL, cat, lang)}
            </button>
          ))}
        </div>
      </div>
      ...
      {products === null ? (
        ...
        <EmptyState title={t('pages.catalog.noResultsTitle')} message={t('pages.catalog.noResultsMsg')} />
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {filtered.map((p) => (
            <Link key={p.id} to={`/productos/${p.id}`}>
              <Card className="overflow-hidden hover:border-gold-500/50 transition-colors h-full flex flex-col">
                ...
                <div className="p-4 flex-1 flex flex-col">
                  <p className="text-[11px] text-gold-600 mb-1">{labelFor(PRODUCT_CATEGORY_LABEL, p.category, lang)}</p>
                  ...
                  <div className="mt-auto flex items-center justify-between">
                    ...
                    {p.stock === 0 ? <Badge tone="red">{t('pages.catalog.outOfStock')}</Badge> : p.stock <= p.minStock ? <Badge tone="amber">{t('pages.catalog.lowStock')}</Badge> : null}
                  </div>
                </div>
              </Card>
            </Link>
